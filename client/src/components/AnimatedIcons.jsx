  ...
  return (
    <svg viewBox="0 0 40 40" fill="none" className={className} style={{ width: size, height: size }}>
      <motion.path
        ...
        stroke={filled ? "#EF4444" : color} strokeWidth={2}
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-16">
      <div className="text-center mb-12">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Animated State Icons</h2>
        <p className="text-sm text-gray-500 max-w-md mx-auto">
          12 icons that morph between two meaningful states — loading→success, play→pause, lock→unlock.
        </p>
      </div>
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-8 justify-items-center">
        {ALL_ICONS.map(({ name, Icon }) => (
          <div key={name} className="flex flex-col items-center gap-3">
            <div className="flex items-center justify-center w-20 h-20 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 text-emerald-600 dark:text-emerald-400">
              <Icon size={36} />
            </div>
            <span className="text-[11px] font-medium text-gray-400 tracking-wide text-center">{name}</span>
          </div>
        ))}
      </div>
    </div>
